...
def gCheck():
    global count3, freeGroups, takenGroups # show me error 
    ...
    with open("gen-output.txt","r") as f:
        ...
        for line in lines:
            ...
            if line == "":
                continue
            global success # you don't need this? you might be right xD idk is late for me : also text is not a function  lets jsut try this
            response = requests.get(f"https://steamcommunity.com/actions/AvailabilityCheck?&type=groupLink&value={line}").text
            #print(response.text) hahahahahahhaa
            #string_xml = response.content # I think this is where it's printing for some reason. || Yes it's on the line 174 wait we could make this easier on ourselves xD
            #tree = xml.etree.ElementTree.fromstring(string_xml)
            #xml.etree.ElementTree.dump(tree) # is this whole thing using xml? or jsut this request? || Open link in browser wait
            ...
            if f"The group link, {line}, is already in use by another group" in response:
                ...
                with open('free-groups.txt', 'a') as fgList:
                    fgList.write(f"{line}\n")
                    takenGroups += 1
            # it was giving that every time it was free so i remove it
...
# Generate and check group vanity's
    if mode == "5":
        ...
        print(Center.XCenter(Colorate.Horizontal(Colors.white_to_green, f"   \nDone! - Free: {free} - Taken: {taken} - Generated: {count} - Checked: {count3} usernames - " + "Elapsed: {}".format(round(elapsed2)) + " seconds", 1)))
        time.sleep(1)
        print(Center.XCenter(Colorate.Horizontal(Colors.white_to_red, "   \nPress any key to go back to main menu", 1)))
        start()
